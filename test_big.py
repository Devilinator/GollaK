...
def test_big(args):
    ...
    names_path = "E:\gig-dl.ir\Dataset/test"
    print("Names to test: {}".format(names_path))

    dataset_path = "E:\\gig-dl.ir\\Dataset"
    print("Dataset: {}".format(names_path))

    weights_path = "E:\Hard Talebi (Gaze Estimation)\GazeEstimation\SE-ResNext\ResNext_weights/weights.001-0.80115.hdf5"
    ...
    for it in list(range(0, len(test_names), chunk_size)):
        ...
        for i, prediction in enumerate(predictions):
            ...
            if(((prediction[0] - y[i][0])**2 + (prediction[1] - y[i][1])**2) >= 20):
                 print("element number should be deleted = {}". i)
