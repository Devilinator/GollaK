...
def load_batch_from_names(names, path, img_ch, img_cols, img_rows):
    ...
    for i, img_name in enumerate(names):
        ...
        # print("No Error opening image: {}".format(join(path, dir, "frames", frame)))
        ...
        # print("No Error with coordinates: {}".format(join(path, dir, "frames", frame)))
...
def load_batch_from_names_random(names, path, batch_size, img_ch, img_cols, img_rows):
    ...
    while b < batch_size:
        ...
        if frame[:-4] == 0:
            print("Error with coordinates: {}".format(join(path, dir, "frames", frame)))
        idx = int(frame[:-4])
        # idx = int.from_bytes(frame[:-4], byteorder='big')
...
if __name__ == "__main__":
    ...
    # debug
    seq_list = load_data_names("D:\Dataset\GazeCapture_5/test")
